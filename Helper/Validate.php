...
namespace Droppery\Core\Helper;
...
//    const DEV_ENV = ['localhost', 'dev', '127.0.0.1', '192.168.', 'demo.'];
    ...
    {
        if ($this->_dropperyModules === null) {
            $this->_dropperyModules = [];
            ...
            if ($allowList) {
                ...
                foreach ($moduleList as $name) {
                    if (strpos($name, 'Droppery_') === false) {
                        continue;
                    }

                    $this->_dropperyModules[] = $name;
                }
            }
        }

        return $this->_dropperyModules;
    }
}
